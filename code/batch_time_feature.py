...
def time_task_gene_awgn(case, tar_dataset_name, data_space, data_type, experiment_time):
    # 分割时域数据，泛化，加噪
...
def time_task_gene(case, tar_dataset_name, data_space, data_type, experiment_time):
    # 分割时域数据，泛化
...
def time_task_awgn(case, tar_dataset_name, data_space, data_type, experiment_time):
    # 分割时域数据，加噪
...
def time_task(case, tar_dataset_name, data_space, data_type, experiment_time):
    # 分割时域数据
...
def clac_task(case, tar_dataset_name, data_type, experiment_time):
    # 得到时域数据的特征18维度
    os.system('python feature_extract.py --case {} --tar_dataset_name {} --data_type {} --experiment_time {}'.format(case, tar_dataset_name, data_type, experiment_time))
...
if __name__ == "__main__":
    ...
    for tar_dataset_name in args.tar_dataset_name_list:
        ...
        if args.generalization and args.add_awgn:
            p.apply_async(time_task_gene_awgn, args=(case, tar_dataset_name, space, 'time', experiment_time, ))
        elif args.generalization:
            p.apply_async(time_task_gene, args=(case, tar_dataset_name, space, 'time', experiment_time, ))
        elif args.add_awgn:
            p.apply_async(time_task_awgn, args=(case, tar_dataset_name, space, 'time', experiment_time, ))
        else:
            p.apply_async(time_task, args=(case, tar_dataset_name, space, 'time', experiment_time, ))
    ...
    for tar_dataset_name in args.tar_dataset_name_list:
        if tar_dataset_name in ['A', 'B', 'C']:
            ...
            case = 'Bearing'
            space = args.split_spcae_canda
        clac_task(case, tar_dataset_name, 'feature', experiment_time)