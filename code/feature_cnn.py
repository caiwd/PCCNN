...
def cnn_features(model, loader, parent_save_path, tar_dataset_name, class_num):
...
if __name__ == "__main__":
    ...
    # logging.warning('提取CNN特征[{}-{}]...'.format(tar_dataset_name, class_num))
    ...
    if torch.cuda.is_available() and args.use_cuda:
        ...
        args.model_path, tar_dataset_name, class_num, 1 if args.add_awgn else experiment_time,
        'gene' if args.generalization else 'baseline'))
    else:
        trained_model = torch.load(r'{}/{}_cnn_model_class_num({})_experiment_time({})_{}.pkl'.format(
            args.model_path, tar_dataset_name, class_num, 1 if args.add_awgn else experiment_time,
            'gene' if args.generalization else 'baseline'), map_location=torch.device('cpu'))
    ...
    for train_test in ['train', 'test']:
        ...
        if train_test == 'train':
            cnn_features(model, train_loader, parent_save_path,
                         tar_dataset_name, class_num)
        elif train_test == 'test':
            cnn_features(model, test_loader, parent_save_path,
                         tar_dataset_name, class_num)
