...
def one_cloumn_list(result, category, tar_dataset_name, feature_list, method_list, std=True):
    ...
    for index, feature in enumerate(feature_list):
        if std:
            temp = [round(result[1][feature][method][tar_dataset_name][category], 2) for method in method_list]
        else:
...
if __name__=='__main__':
    ...
    for index in range(1):
        ...
        for indexs, tar_dataset_name in enumerate(args.tar_dataset_name_list):
            ...
            dict_datas['Aver_known'] = np.round(dict_datas['Aver_known'], 2)
            dict_datas['Aver_unknown'] = np.round(dict_datas['Aver_unknown'], 2)
            dict_datas['Aver_aver'] = np.round(dict_datas['Aver_aver'], 2)

        df = pd.DataFrame(dict_datas, index=['{}_{}'.format(feature, method) for feature in feature_list for method in method_list])

        df.to_excel(r'{}/更新前实验结果_标准差_{}_{}.xlsx'.format(args.result_path,
                                'gene' if args.generalization else 'baseline',
                                'noise' if args.add_awgn else 'no_noise'))
    ...
    for index in range(1):
        ...
        df.to_excel(r'{}/更新前实验结果_准确率_{}_{}.xlsx'.format(args.result_path,
                                'gene' if args.generalization else 'baseline',
