...
class SVDD():
    ...
    def fit(self, data):
        ...
        """
        label = np.ones(shape=(len(data), 1))
    ...
    def predict(self, data):
        ...
        # distance
        distance = -1*np.sqrt(np.diagonal(term_1+term_2+term_3))
        
        # predicted label
        predicted_label = np.ones(n)
        fault_index = np.where(distance < -1*self.model["radius"][0, 0])[0]
        if len(fault_index)>0:
            predicted_label[fault_index] = -1
